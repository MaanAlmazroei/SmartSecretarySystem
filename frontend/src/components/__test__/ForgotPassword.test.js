...
describe("ForgotPassword Component - handleChange", () => {
  ...
  test("shows error toast when email sending fails", async () => {
    sendPasswordResetEmail.mockRejectedValueOnce(new Error("Failed to send reset link"));

    const emailInput = screen.getByLabelText(/email/i);
    const submitButton = screen.getByText(/send reset link/i);
    // writing an email
    fireEvent.change(emailInput, { target: { value: "official.hatimalharbi@gmail.com" } });
    ...
    expect(sendPasswordResetEmail).toHaveBeenCalledWith({ mockAuth: true }, "official.hatimalharbi@gmail.com");
