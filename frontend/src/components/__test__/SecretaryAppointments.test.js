...
describe("SecretaryAppointments Component", () => {
    ...
    test("filters appointments by status", async () => {
      ...
      expect(await screen.findByText(/No appointments found matching your criteria/i)).toBeInTheDocument();
    });
    ...
    test("searches appointments by title", async () => {
      render(<SecretaryAppointments />);
      await screen.findByText("Test Apoint");
      fireEvent.change(screen.getByLabelText(/Search Appointments/i), { target: { value: "notfound" } });
      expect(await screen.findByText(/No appointments found matching your criteria/i)).toBeInTheDocument();
      fireEvent.change(screen.getByLabelText(/Search Appointments/i), { target: { value: "Test" } });
      expect(await screen.findByText("Test Apoint")).toBeInTheDocument();
    });
    ...
    test("updates appointment status", async () => {
      ...
      await waitFor(() => expect(ApiService.updateAppointment).toHaveBeenCalledWith("1", { status: "Approved" }));
    });
    ...
    test("adds feedback to appointment", async () => {
      render(<SecretaryAppointments />);
      fireEvent.click(await screen.findByText("Test Apoint"));
      fireEvent.change(screen.getByPlaceholderText(/Type your feedback/i), { target: { value: "Great job" } });
      ...
      await waitFor(() => expect(ApiService.updateAppointment).toHaveBeenCalledWith("1", expect.objectContaining({ feedback: "Great job" })));
    });
