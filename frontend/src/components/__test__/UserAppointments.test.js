...
describe("UserAppointments Component", () => {
  ...
  test("renders form and handles input changes", () => {
    ...
    expect(screen.getByLabelText(/Appointment Title/i).value).toBe(
      "Test Title"
    );
  });
  ...
  test("shows error for short description", async () => {
    ...
    expect(
      await screen.findByText(/Description should be at least 10 characters/i)
    ).toBeInTheDocument();
  });
  ...
  test("submits form and resets on success", async () => {
    ...
    await waitFor(() =>
      expect(ApiService.createAppointment).toHaveBeenCalled()
  ...
  test("shows error message if createAppointment API fails", async () => {
    ...
    expect(
      await screen.findByText(/Error creating appointment:/i)
    ).toBeInTheDocument();
  });
});
